...
**按需截图无需接入编程助手。** 询问画面时，“画面分析”子智能体会分析一张截图，再将文字结果交给 Omni 回答。在**子智能体**窗口中可查看状态和结果，或停止分析；结束通话会取消未完成的分析。它使用主对话的模型、API key 和地域，不需要另选模型。
...
如果搜索请求失败（包括服务端不支持原生搜索），已配置的编程助手会接手同一个只读查询；没有编程助手时，会提示查询失败。转交使用独立任务，不会打断其他正在运行的任务，也不代替你批准权限。
