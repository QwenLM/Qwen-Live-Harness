# Qwen Live Harness visual input — Source × Mode
...
model needs it. `qwen-live-harness init` persists these defaults without prompting for
...
- `QWEN_LIVE_HARNESS_VISUAL_SOURCE`
- `QWEN_LIVE_HARNESS_VISUAL_MODE`
...
Appshot. The qwen-live-harness and CLI protocol type files remain byte-identical.

## Diagnostics and verification

`qwen-live-harness --debug` logs privacy-safe configuration, connection, call, capture,
frame acceptance, and provider error metadata to stderr. The Host executable
also accepts `--live-harness-debug` for capture-side dimensions, byte counts, and error
