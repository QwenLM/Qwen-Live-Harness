# Qwen Live Harness Proactive

## Goal

Port the in-call Proactive capability from `qwen-omni-realtime-agent` into
Qwen Live Harness. The port supports only the DashScope Realtime monitor backend,
...
`QWEN_LIVE_HARNESS_PROACTIVE_ENABLED` is the environment override for the master
...
call, Qwen Live Harness asks the same foreground model to re-evaluate that turn in one
...
evidence always follows Qwen Live Harness's currently selected Screen or Camera source:
