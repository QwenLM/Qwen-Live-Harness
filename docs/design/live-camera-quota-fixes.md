...
`camera_snapshot_resolution_unavailable`. A prior physical-device report showed
...
localized startup/active-call guidance. Validation in this repository reproduces the reported dimensions with synthetic camera inputs and verifies the compiled daemon against local provider sockets. These checks do not invoke physical media or the real provider, and do not claim a complete spoken camera conversation.
