...
- If a cancelled non-direct response never receives created or done, retire its socket before reporting `response_cancelled_timeout`. Do not clear arbitration and resend on that socket: request correlation has not been established for the configured provider.
- Recover this cancellation timeout at most once per Live call, using the same endpoint, model and voice. Preserve the Host call, backend jobs, permissions, queued results and Memory object; fence old callbacks and asynchronous tool receipts by connection generation. Restore the bounded transcript tail as historical context, never as a new action request.
- Never replay a user request or tool. Audio during recovery has no replay guarantee, so show and speak a request to repeat the last utterance. A second cancellation timeout, an uncancelled response timeout or failed reconnect still fails explicitly instead of looping.

## Boundaries

No new process, provider, model, media permission, automatic capture, or permanent image archive is introduced. On Demand image bytes now go to the same Realtime provider already receiving live audio and Live Feed frames, using its existing JPEG input channel and size limits. This changes visual inference billing from the configured backend to the current Realtime model for ordinary visual questions; the cost difference is not measured. Optional local image assets remain available for explicit backend tasks with the same temporary-file lifetime. Cancellation recovery adds at most one connection to the existing provider and does not repeat backend tasks. Receipt guidance improves model behavior but is not a deterministic guarantee of model adherence.
