...
export class FakeHost {
  ...
  autoCompletePlayback = true;

  private socket: WebSocket | undefined;
  private readonly pendingPlayback = new Set<string>();
  ...
  async connect(timeoutMs = 10_000): Promise<void> {
    ...
    socket.on('message', (data, isBinary) => {
      if (isBinary) {
        ...
        const epoch = Number(frame.readBigUInt64BE(0));
        const outputId = Number(frame.readBigUInt64BE(8));
        const key = `${epoch}:${outputId}`;
        if (!this.pendingPlayback.has(key)) {
          this.pendingPlayback.add(key);
          this.send({ type: 'host.playback_started', epoch, outputId });
        }
      ...
      } else if (message['type'] === 'host.clear_output') {
        this.pendingPlayback.clear();
      } else if (
        message['type'] === 'host.output_audio_finished' &&
        this.autoCompletePlayback
      ) {
        this.completePlayback(
          Number(message['epoch']),
          Number(message['outputId']),
        );
      } else if (
...
export async function startLiveCall(
  stack: Pick<LiveStack, 'host' | 'fakeDash'>,
): Promise<{
  epoch: number;
...
export async function bootAcpLiveStack(
  ...
  try {
    ...
    const acpBackend = {
      ...
      args: [qwenCliPath(), '--acp', '--no-chat-recording'],
      env: {
        HOME: homeDir,
        QWEN_HOME: qwenHome,
        OPENAI_API_KEY: 'fake-key',
