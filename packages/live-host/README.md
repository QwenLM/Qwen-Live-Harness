...
Qwen Live Host 是独立 Qwen Live daemon 在 macOS 上的原生组件。它承载小浮层、
...
- macOS 12 或更高版本。
- 本机运行的独立 `qwen-live` daemon。
- 可调用 `qwen3.5-omni-plus-realtime` 的 DashScope API key。

`qwen-live` 可直接从命令行启动并连接 Host；编码后端按配置通过 ACP 或 REST/SSE 接入。
不需要 qwen-code 源码或 `qwen serve`。原生音视频功能目前要求 macOS。

## 首次启用

1. 运行 `qwen-live init`，配置语言、DashScope API key 和可用编码后端。
2. 运行 `qwen-live` 启动 daemon；全局快捷键默认是 `Command+E`。
...
API key 保存在用户级 `~/.qwen-live/config.json`。停止 daemon 会结束当前通话并撤下
Host discovery；不会卸载 Host 或删除 Live 对话。迁仓沿用配置、数据目录和 Host 权限身份。
...
`packages/live-host/release/`。Host 的依赖树单独安装，daemon 安装不下载 Electron。
从仓库根目录运行 daemon 构建不需要构建 Host；修改共用协议、文案或子任务类型后需分别验证两个包。

## 发布

本仓库使用 **Qwen Live Host Release** workflow，daemon 和 Host 采用配套版本。
...
仓库目前为 private，GitHub release 无法被未登录用户下载。公共安装依赖现有 OSS
`https://qwen-code-assets.oss-cn-hangzhou.aliyuncs.com/live-host` 前缀由本仓库发布；
它的名称是兼容下载地址，不需要 qwen-code checkout。GitHub fallback 仅在源仓库对
用户可访问时可用。切换发布归属前应下线原仓库的 Host 发布/镜像 workflow，避免两个
仓库同时更新同一 stable feed。已有公开 0.2.0 Host 是 protocol v7；本仓库首版 0.3.0
为 v9，需要先发布新的签名 Host，不能把旧 feed 当作迁移后的首次安装验证。
...
- npm：`production-release` environment 中的 `NPM_TOKEN`，需要拥有
  `@qwen-code/qwen-live` 的发布权限。公开源码发布启用 provenance；private 源码发布
  使用 token，不生成公开源码 provenance。

保留 bundle ID `com.alibaba.qwen-code.live-host`、Developer ID team `NF4574S59H`、
产品名和 `/Applications/Qwen Live Host.app` 路径，使现有 macOS 权限身份继续适用。
只通过签名产物安装验证权限延续，未签名开发构建不代表验证通过。
