...
The daemon sends an On Demand snapshot to a read-only visual-analysis subagent, then passes its text result to the main conversation. Host displays progress and results in Subagents; it does not call the model itself. Live Feed and Proactive use separate input paths. See the [configuration guide](../../docs/configuration.md) for sources, modes, resolutions, and limitations.
