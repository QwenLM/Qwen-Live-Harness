# Qwen Live Harness Host

Qwen Live Harness Host 是独立 Qwen Live Harness daemon 在 macOS 上的原生组件。它承载小浮层、
...
请先退出旧 daemon 和 Host，重新 init；新 bundle ID 需要重新确认 macOS 授权。
...
```bash
cd packages/qwen-live-harness-host
npm ci
npm run build
npm run typecheck
npm test
...
构建产物位于 `packages/qwen-live-harness-host/dist/`，打包产物位于
`packages/qwen-live-harness-host/release/`。Host 的依赖树单独安装，daemon 安装不下载 Electron。
从仓库根目录运行 daemon 构建不需要构建 Host；修改共用协议、文案或子任务类型后需分别验证两个包。
...
本仓库使用 **Qwen Live Harness Host Release** workflow，daemon 和 Host 采用配套版本。
PR 会自动执行一次未签名 dry run，
检查 arm64/x64 的 DMG、ZIP 和 manifest；正式发布只能从 `main` 手工触发，并执行
Developer ID 签名、notarization、Gatekeeper 和 stapler 验证。

版本发布使用 `qwen-live-harness-host-vX.Y.Z` tag，包含两个 DMG、两个 ZIP、
`Qwen-Live-Harness-Host-manifest.json` 和 `SHA256SUMS.txt`。非 draft、非 prerelease 的
正式版本还会更新固定的 GitHub `qwen-live-harness-host-latest` feed，并在发布成功后调用一次
独立的 OSS 镜像 workflow。镜像保存版本化 ZIP 和 manifest，再发布一个 latest
manifest；需要重传时可手工运行同一镜像 workflow。自动安装优先读取 OSS，
失败时使用本仓库的 GitHub feed。

发布前必须在源码中同步更新两个 package.json 及 lock 文件版本；workflow 输入必须与
提交版本一致。已公开 release 和 npm 版本不可覆盖；`clobber` 仅用于更新尚未公开的
GitHub draft。draft 不发布 npm；稳定版先完成 Host 和公共 OSS 分发，再发布 npm。
`preview` npm 标签用于 prerelease；安装器的稳定 Host feed 不随 prerelease 更新。

若仓库为 private，GitHub release 无法被未登录用户下载。公共安装依赖 OSS 的新
...
- 公证：`APPLE_NOTARY_ISSUER_ID`、`APPLE_NOTARY_KEY_ID`、
  `APPLE_NOTARY_API_KEY_P8_BASE64`，或 `APPLE_API_ISSUER`、`APPLE_API_KEY`、
  `APPLE_API_KEY_P8`；两种方式都需要 `APPLE_TEAM_ID`。
...
Developer ID team `NF4574S59H` 是实际签名者身份，保持不变；bundle ID 改变后不承诺
旧 macOS 权限继承。用户需要重新授予所需权限；未签名开发构建不代表签名安装验证通过。
...
`~/.qwen-live-harness/config.json`，也支持 daemon 的 `QWEN_LIVE_HARNESS_DATA_DIR`）。保存后需重启
Qwen Live Harness 才应用手动修改。旧 daemon 或内置 `qwen serve` 不提供此入口能力；文件
...
独立 daemon 确认后立即切换，并保存到 `~/.qwen-live-harness/config.json` 顶层 `language`
...
自定义名称。`qwen-live-harness init` 的第一项也可通过左右方向键选择语言，后续问题随之切换。

全部固定 Live 展示文案统一维护在
[`packages/qwen-live-harness/src/i18n/messages.ts`](../qwen-live-harness/src/i18n/messages.ts)，
每个键并列 `en` 和 `zh-CN`。修改后分别重建 Live 和 Host。Host 的构建别名直接编译
同一份纯文本模块，打包后不需要 qwen-live-harness 运行时依赖；开发构建仍应在完整仓库内进行。
...
连接独立 Qwen Live Harness daemon 时，Settings 的 `Memory` 区域提供记忆开关、
...
内联编辑草稿不会被音频状态刷新打断。库默认存储在 `~/.qwen-live-harness/memories`。
daemon 断开时 Settings 关闭，未保存的编辑草稿保留到重新连接后再次打开。
`qwen-live-harness init` 也会询问是否启用 Memory 和整理模型名，详细参数见
[Memory 文档](../qwen-live-harness/README.md#memory)。旧的 WebShell 内置 Live 没有声明 Memory
能力时，Host 不显示这一入口。
...
`QWEN_LIVE_HARNESS_CAMERA_SNAPSHOT_RESOLUTION=native` 或 `WIDTHxHEIGHT`。Host 优先从同一
...
菜单栏、Dock 和其他应用，但排除 Qwen Live Harness Host 自身窗口。Settings 的 Video Source 下可选
...
开发诊断可在终端启动开发版或应用可执行文件并传入 `--live-harness-debug`。不要给 Electron
...
```bash
cd packages/qwen-live-harness-host
npm start -- --live-harness-debug

"release/mac-arm64/Qwen Live Harness Host.app/Contents/MacOS/Qwen Live Harness Host" --live-harness-debug
```

Proactive 判断、通知排队／播报、harness 任务和 Realtime 生命周期日志由 **daemon** 输出，
需在另一个终端运行 `qwen-live-harness --debug`。Host 的 `--live-harness-debug` 不会替代 daemon 的日志开关。
...
`qwen-live-harness-monitor-debug/`。每个 Monitor 一个目录，每次推理保存 `request.json`、
...
仅 daemon debug 开启；Host 的 `--live-harness-debug` 单独启用不会录制，纯音频 Monitor 也不录制。
...
完整格式与清理规则见 [Qwen Live Harness README](../qwen-live-harness/README.md)。
...
| 权限     | 授权主体               | 用途                                   |
| -------- | ---------------------- | -------------------------------------- |
| 麦克风   | Qwen Live Harness Host | 采集 Live 对话音频                     |
| 摄像头   | Qwen Live Harness Host | Camera Source 的预览、实时帧或单帧截图 |
| 辅助功能 | Qwen Live Harness Host | 读取前台窗口的可访问性树               |
| 屏幕录制 | Qwen Live Harness Host | Screen Source 的实时帧或单帧截图       |
...
`--live-harness-debug` 日志中的 `output_context_ready` 显示源／输出上下文采样率及是否重采样。
...
1. 从菜单栏选择“退出 Qwen Live Harness Host”。
2. 如果用户曾手工添加 Login Item，在系统设置中将其移除。
3. 从 `/Applications` 删除 **Qwen Live Harness Host.app**。
4. 在 WebShell 的 **设置 → 实验性功能 → Qwen Live Harness** 中关闭功能。
