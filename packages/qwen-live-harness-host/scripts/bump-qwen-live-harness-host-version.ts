...
try {
  ...
  if (!version || candidate.includes('+')) {
    throw new Error(
      `Invalid Qwen Live Harness Host release version "${input}".`,
    );
  }
  ...
  if (typeof currentVersion !== 'string') {
    throw new Error(
      'Qwen Live Harness Host package does not define a version.',
    );
  }
  ...
  console.log(
    `Qwen Live Harness Host version: ${currentVersion} -> ${version}`,
  );
} catch (error) {
