...
import { liveText, liveMessage } from 'qwen-live-harness/i18n';
...
describe('LiveDaemonConnection', () => {
  it('authenticates, handshakes, handles heartbeat, and enforces binary ownership', async () => {
    ...
    const directory = await mkdtemp(
      join(tmpdir(), 'qwen-live-harness-connection-'),
    );
        ...
        captureVisual: async (request) => {
          ...
          return {
            ...
            ...(request.persistAsset === false
              ...
              : {
                  screenshotPath:
                    '/private/tmp/qwen-live-harness-appshot/visual.png',
                }),
          };
        },
      },
    ...
    assert.equal(
      request.headers['x-qwen-live-harness-nonce'],
      'abcdefghijklmnop',
    );
    ...
    assert.equal(hello.bundleId, 'com.alibaba.qwen-live-harness.host');
  ...
  it('keeps retrying the same discovery identity slowly after the fast budget', async () => {
    ...
    const directory = await mkdtemp(
      join(tmpdir(), 'qwen-live-harness-reconnect-'),
    );
    cleanup.push(() => rm(directory, { recursive: true, force: true }));
    const discoveryPath = join(directory, 'daemon.json');
    await writeFile(
