...
async function discoveryFile(
  ...
  const directory = await mkdtemp(
    join(tmpdir(), 'qwen-live-harness-discovery-'),
  );
...
describe('Live daemon discovery', () => {
  ...
  it('uses only the Harness discovery location and environment override', () => {
    ...
    assert.equal(resolveDiscoveryPath({}), expected);
    assert.equal(
      resolveDiscoveryPath({ QWEN_LIVE_DISCOVERY_FILE: '/legacy/daemon.json' }),
      expected,
    );
    assert.equal(
      resolveDiscoveryPath({
        QWEN_LIVE_HARNESS_DISCOVERY_FILE: '/custom/daemon.json',
      }),
      '/custom/daemon.json',
    );
    assert.equal(
