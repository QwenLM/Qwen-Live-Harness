...
} from '../../../../qwen-live-harness/src/host/types.ts';
import {
  LIVE_HOST_BUNDLE_ID,
...
const DAEMON_PROTOCOL_TYPES_URL = new URL(
  '../../../../qwen-live-harness/src/host/types.ts',
  import.meta.url,
);
...
describe('Qwen Live Harness Host protocol', () => {
  ...
  it('stays synchronized with the daemon protocol contract', async () => {
    ...
    assert.equal(
      Number(
        builderConfig.match(/QwenLiveHarnessProtocolVersion: (\d+)/u)?.[1],
      ),
      LIVE_PROTOCOL_VERSION,
    );
