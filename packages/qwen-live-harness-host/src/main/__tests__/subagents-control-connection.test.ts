...
import { MAX_SUBAGENTS_CONTROL_BYTES } from 'qwen-live-harness/subagents';
...
async function fixture(
  ...
  const directory = await mkdtemp(
    join(tmpdir(), 'qwen-live-harness-subagent-controls-'),
  );
...
describe('standalone subagent management transport', () => {
  ...
  it('uses nonce-authenticated daemon management while the call is idle', async () => {
    ...
    assert.equal(
      value.requests[0]?.headers['x-qwen-live-harness-nonce'],
      nonce,
    );
  });
