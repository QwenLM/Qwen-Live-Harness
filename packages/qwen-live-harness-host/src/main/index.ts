...
app.setName('Qwen Live Harness Host');
...
function openHostAudioCapture(
  ...
  const directory = process.env['QWEN_LIVE_HARNESS_DIAGNOSTICS_DIR'];
  if (
    process.env['QWEN_LIVE_HARNESS_DIAGNOSTICS'] !== '1' ||
    !directory?.trim()
  ) {
    return undefined;
  }
...
function createOverlay(): BrowserWindow {
  ...
  const window = new BrowserWindow({
    ...
    hasShadow: false,
    alwaysOnTop: true,
    title: 'Qwen Live Harness Host',
    webPreferences: {
      preload: join(__dirname, 'preload.cjs'),
      ...(diagnosticsEnabled
        ? { additionalArguments: ['--qwen-live-harness-debug'] }
        : {}),
...
function trayIconPath(): string {
  return app.isPackaged
    ? join(process.resourcesPath, 'qwen-live-harness-host-icon.png')
