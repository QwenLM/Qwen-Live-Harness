...
std::optional<WindowTarget> FindForegroundWindow() {
  ...
  for (NSDictionary* window in windows) {
    ...
    if (app_name.empty() || app_name == "Qwen Live Harness Host" ||
...
napi_value CaptureDisplay(napi_env env, napi_callback_info info) {
  ...
  if (napi_create_async_work(env, nullptr, String(env, "QwenLiveHarnessDisplayCapture"),
...
void ExecuteCapture(napi_env, void* data) {
  ...
  @autoreleasepool {
    if (!AXIsProcessTrusted()) {
      work->error =
          "Qwen Live Harness Host needs Accessibility permission for Appshot.";
      return;
    }
    if (!CGPreflightScreenCaptureAccess()) {
      work->error =
          "Qwen Live Harness Host needs Screen Recording permission for Appshot.";
      return;
    }
...
napi_value CaptureAppshot(napi_env env, napi_callback_info) {
  ...
  napi_value resource_name = String(env, "QwenLiveHarnessAppshot");
