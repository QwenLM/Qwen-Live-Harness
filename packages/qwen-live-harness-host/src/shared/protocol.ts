import { isLiveLanguage, type LiveLanguage } from 'qwen-live-harness/i18n';
import {
...
export function encodeHostControlMessage(message: HostControlMessage): string {
  ...
  if (
    ...
    throw new Error('Invalid Qwen Live Harness Host language action');
  if (
    ...
    throw new Error('Invalid Qwen Live Harness Host memory action');
  }
  if (
    ...
    throw new Error('Invalid Qwen Live Harness Host visual frame');
  }
  if (
    ...
    throw new Error('Invalid Qwen Live Harness Host visual settings');
  }
  if (
    ...
    throw new Error('Invalid Qwen Live Harness Host visual capture');
  }
  if (
    message.type === 'host.visual_capture_result' &&
    message.success &&
    ...
    throw new Error('Invalid Qwen Live Harness Host display capture');
  const encoded = JSON.stringify(message);
  if (Buffer.byteLength(encoded, 'utf8') > MAX_CONTROL_FRAME_BYTES) {
    throw new Error(
      'Qwen Live Harness Host control frame exceeds the protocol limit',
    );
  }
  return encoded;
}
