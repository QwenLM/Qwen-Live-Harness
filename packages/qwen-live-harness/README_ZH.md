...
**任务生命周期。** 由 Omni 理解当前用户输入，决定实际调用哪个创建、修改或取消工具。运行时不使用 ASR 白名单、黑名单或意图分类器，也不把转写文字与任务标题进行语义匹配。ASR 只作为可选的解说语言／风格线索和诊断信息，不作执行门槛。仍检查当前输入与响应身份、参数、目标是否唯一存在、会话／作业是否对应，以及等待期间目标是否被替换；同一输入中的重复调用复用回执。转写或口头承诺本身不会触发自动补调。系统提示仍要求 Omni 区分闲聊与任务请求，意图不清时先澄清。界面停止按钮仍是明确的用户操作；结构校验拒绝记为 `task.authorization_rejected`，不会中断通话。
...
自然表达不要求固定口令。这次调整的是任务工具调用的判断方式，不改变监控的采样频率和检测条件。
...
| 已知模型服务临时错误   | 主连接就绪后，精确的 `COMMON_ERROR / <50002> InternalError.Algo.ModelServingError` 共用上述两次连接恢复额度。保留已有任务，不重放已派发工具、回执或图片，原始错误仍记入诊断。其他服务端错误沿用各自处理。 |
