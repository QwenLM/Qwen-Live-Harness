...
async function temporaryDirectory(): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'qwen-live-harness-daemon-'));
...
describe('LiveDaemon', () => {
  ...
  it('authenticates standalone subagent management by bearer and instance without an active call', async () => {
    ...
    expect(
      (
        await requestPage({
          ...hostHeaders(record),
          'x-qwen-live-harness-nonce': 'previous-instance',
        })
      ).status,
    ).toBe(409);
  ...
  it('wires active-epoch playback receipts and gracefully quits during a call', async () => {
    ...
    if (!call) throw new Error('Qwen Live Harness call did not start');
    const coordinator = (
      daemon as unknown as {
        coordinator?: {
  ...
  it('refuses shutdown without authentication or with the wrong instance', async () => {
    ...
    for (const [headers, status] of [
      ...
      [
        {
          ...hostHeaders(record),
          'x-qwen-live-harness-nonce': 'wrong-instance',
        },
        409,
      ],
      [
  ...
  it.each(['session', 'backend'] as const)(
    ...
    async (failure) => {
      ...
      expect(response.status).toBe(500);
      expect(await response.json()).toEqual({
        error: 'Qwen Live Harness shutdown cleanup failed.',
      });
  ...
  it('fails fast on a live discovery owner instead of stealing the file', async () => {
    ...
    await expect(daemon.start()).rejects.toThrow(
      /Another Qwen Live Harness daemon \(pid \d+\) already owns/,
    );
