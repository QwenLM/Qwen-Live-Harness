...
async function temporaryRuntime(): Promise<string> {
  const directory = await fs.mkdtemp(
    path.join(os.tmpdir(), 'qwen-live-harness-'),
  );
...
describe('Qwen Live Harness discovery file', () => {
  it('uses only the Harness discovery namespace', () => {
    const base = getStableLiveDiscoveryBaseDir('/synthetic-home');
    expect(base).toBe(path.join('/synthetic-home', '.qwen-live-harness'));
    expect(getLiveDiscoveryPath(base)).toBe(
      path.join('/synthetic-home', '.qwen-live-harness', 'run', 'daemon.json'),
    );
    expect(getLiveDiscoveryPath('/custom-discovery')).toBe(
      path.join('/custom-discovery', 'run', 'daemon.json'),
    );
  });
  ...
  it('does not overwrite an invalid existing record', async () => {
    ...
    await expect(
      ...
      'Existing Qwen Live Harness discovery record is invalid.',
    );
