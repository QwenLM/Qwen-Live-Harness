...
export class LiveDiscoveryOwnerActiveError extends Error {
  constructor(readonly ownerPid: number) {
    super(
      `Qwen Live Harness discovery is owned by active daemon pid ${ownerPid}.`,
    );
...
export class LiveDiscoveryStateError extends Error {
  constructor(cause?: unknown) {
    super('Existing Qwen Live Harness discovery record is invalid.', { cause });
...
export class LiveDiscoveryPublicationError extends Error {
  ...
  constructor(cause: unknown) {
    super(
      'Qwen Live Harness discovery publication completed with cleanup errors.',
      {
        cause,
      },
    );
...
export function getStableLiveDiscoveryBaseDir(
  ...
  return defaultLiveDataDirectory(homeDirectory);
}
...
function assertSafeRecord(record: ExistingLiveDiscoveryRecord): void {
  if (
    ...
    throw new Error('Qwen Live Harness discovery record is invalid.');
  }
  let url: URL;
  try {
    url = new URL(record.url);
  } catch {
    throw new Error('Qwen Live Harness discovery URL is invalid.');
  }
  ...
  if (
    ...
    throw new Error(
      'Qwen Live Harness discovery URL must be a loopback HTTP URL.',
    );
  }
  if (
  ...
  try {
    ...
    throw new LiveDiscoveryStateError(
      releaseError
        ? new AggregateError(
            [error, releaseError],
            'Qwen Live Harness discovery lock validation cleanup failed.',
          )
        : error,
    );
  }
    ...
    throw new LiveDiscoveryStateError(
      operationError
        ? new AggregateError(
            ...
            'Qwen Live Harness discovery handoff cleanup failed.',
          )
        : releaseError,
    );
  }
...
export async function writeLiveDiscoveryFile(
  ...
  try {
    ...
    if (Buffer.byteLength(serialized) > MAX_DISCOVERY_BYTES) {
      throw new Error('Qwen Live Harness discovery record is too large.');
    }
    ...
    const cause =
      cleanupErrors.length > 0
        ? new AggregateError(
            ...
            'Qwen Live Harness discovery publication cleanup failed.',
          )
        : operationError;
