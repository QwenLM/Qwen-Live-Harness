...
} from './qwen-live-harness-host-coordinator.js';
import {
  LIVE_HOST_BUNDLE_ID,
...
describe('LiveHostCoordinator', () => {
  ...
  it('routes one correlated On Demand capture only for the active Qwen Live Harness session', async () => {
    const value = coordinator();
    const socket = connectReady(value);
    const call = value.start('resume');
    value.setCoordinator(call.epoch, {
    ...
    await expect(value.captureVisualContext('worker-1')).rejects.toThrow(
      'active Qwen Live Harness session',
    );
    const capture = value.captureVisualContext('coordinator-1');
    const request = socket
      .messages()
  ...
  it('captures Camera on demand without requiring Screen readiness', async () => {
    ...
    socket.receive({
      ...
      width: 1920,
      height: 1080,
      screenshotPath: '/private/tmp/qwen-live-harness-appshot/test.jpg',
    });
  ...
  it('lets the active Qwen Live Harness session finish On Demand capture during stop drain', async () => {
  ...
  it('can turn the global shortcut off without disabling Qwen Live Harness', async () => {
    const value = coordinator();
    const socket = connectReady(value);
  ...
  it('owns one call epoch and ignores stale updates and actions', () => {
    ...
    expect(socket.messages()).toContainEqual({
      type: 'host.error',
      code: 'stale_epoch',
      message: 'The Qwen Live Harness action epoch is stale.',
    });
  ...
  it('rejects removed Host messages', () => {
    ...
    for (const message of removedMessages) {
      ...
      expect(socket.messages()).toContainEqual({
        ...
        message: 'Invalid Qwen Live Harness Host message.',
      });
    }
  });
