...
export class LiveUnavailableError extends Error {
  ...
  constructor(readonly status: LiveStatus) {
    super(status.message ?? 'Qwen Live Harness voice is unavailable.');
...
export class LiveHostCoordinator {
  ...
  setConfiguredShortcut(shortcut: string): LiveStatus {
    ...
    if (normalized.length > MAX_SHORTCUT_LENGTH) {
      throw new Error('The Qwen Live Harness shortcut is too long.');
    }
  ...
  setShortcut(shortcut: string): Promise<LiveStatus> {
    ...
    if (normalized.length > MAX_SHORTCUT_LENGTH) {
      return Promise.reject(
        new Error('The Qwen Live Harness shortcut is too long.'),
      );
    }
    ...
    if (this.pendingShortcut) {
      return Promise.reject(
        new Error(
          'Another Qwen Live Harness shortcut update is already in progress.',
        ),
      );
    }
    ...
    if (!host?.hello || !this.isLeaseHealthy(host)) {
      return Promise.reject(
        new Error(
          'Qwen Live Harness Host must be connected to change the shortcut.',
        ),
      );
    }
    ...
    return new Promise<LiveStatus>((resolve, reject) => {
      const timer = setTimeout(() => {
        this.rejectPendingShortcut(
          new Error(
            'Qwen Live Harness Host did not confirm the shortcut change.',
          ),
        );
      ...
      if (
        ...
        this.rejectPendingShortcut(
          new Error(
            'Qwen Live Harness Host is unavailable for shortcut changes.',
          ),
        );
      }
    });
  ...
  captureVisualContext(
    ...
    if (
      ...
      return Promise.reject(
        new Error(
          'On Demand capture is available only to the active Qwen Live Harness session with a ready selected source.',
        ),
      );
    }
    const requestId = randomUUID();
    ...
    return new Promise<LiveVisualCapture>((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(
          new Error('Qwen Live Harness Host On Demand capture timed out.'),
        );
      }, this.visualCaptureTimeoutMs);
      timer.unref?.();
      ...
      if (
        ...
        this.rejectPendingVisualCapture(
          requestId,
          new Error(
            'Qwen Live Harness Host is unavailable for On Demand capture.',
          ),
        );
      }
    });
  ...
  dispose(): void {
    ...
    this.rejectPendingVisualCaptures(
      new Error('Qwen Live Harness voice is shutting down.'),
    );
    this.rejectPendingShortcut(
      new Error('Qwen Live Harness voice is shutting down.'),
    );
  ...
  private handleTextFrame(lease: HostLease, data: RawData): void {
    ...
    if (!message) {
      ...
      lease.socket.close(1002, 'Invalid Qwen Live Harness Host message.');
      return;
    }
  ...
  private handleShortcutResult(message: LiveHostShortcutResult): void {
    ...
    if (message.shortcut !== pending.shortcut) {
      pending.reject(
        new Error('Qwen Live Harness Host returned a mismatched shortcut.'),
      );
      return;
    }
    if (!message.success) {
      ...
      pending.reject(
        new Error(
          message.error ||
            'The Qwen Live Harness shortcut could not be registered.',
        ),
      );
      return;
    }
  ...
  private handleVisualCaptureResult(
    ...
    if (!call || call.epoch !== pending.epoch) {
      this.rejectPendingVisualCapture(
        message.requestId,
        new Error(
          'The Qwen Live Harness call changed before visual capture completed.',
        ),
      );
      return;
    }
    ...
    if (message.source !== pending.source) {
      ...
      pending.reject(
        new Error(
          'Qwen Live Harness Host returned the wrong visual capture source.',
        ),
      );
      return;
    }
    ...
    if (pending.persistAsset && !message.screenshotPath) {
      ...
      pending.reject(
        new Error(
          'Qwen Live Harness Host did not persist the requested visual capture.',
        ),
      );
      return;
    }
  ...
  private handleAction(action: LiveHostAction): void {
    if (
      ...
      this.sendHostError(
        'stale_epoch',
        'The Qwen Live Harness action epoch is stale.',
      );
      return;
    }
  ...
  private finishCall(call: LiveCall): void {
    ...
    this.rejectPendingVisualCaptures(
      new Error(
        'The Qwen Live Harness call ended before visual capture completed.',
      ),
      call.epoch,
    );
    ++this.nextEpoch;
  ...
  private detachHost(
    ...
    this.rejectPendingVisualCaptures(
      new Error('Qwen Live Harness Host disconnected.'),
    );
    this.rejectPendingShortcut(
      new Error('Qwen Live Harness Host disconnected.'),
    );
