...
export const LIVE_HOST_BUNDLE_ID = 'com.alibaba.qwen-live-harness.host';
export const LIVE_HOST_TEAM_IDENTIFIER = 'NF4574S59H';
export const LIVE_HOST_APP_PATH = '/Applications/Qwen Live Harness Host.app';
...
export const LIVE_HOST_MANIFEST_NAME = 'Qwen-Live-Harness-Host-manifest.json';
export const LIVE_HOST_MANIFEST_FETCH_TIMEOUT_MS = 5 * 60 * 1000;
export const LIVE_HOST_DOWNLOAD_TIMEOUT_MS = 60 * 60 * 1000;

const LIVE_HOST_APP_NAME = 'Qwen Live Harness Host.app';
...
export function parseLiveHostReleaseManifest(
  ...
  return {
    ...
    assets: {
      arm64: parseAsset(
        value['assets']['arm64'],
        'Qwen-Live-Harness-Host-arm64.zip',
      ),
      x64: parseAsset(value['assets']['x64'], 'Qwen-Live-Harness-Host-x64.zip'),
    },
  };
}
...
async function installLatestHost(
  ...
  const temporaryDirectory = await fsp.mkdtemp(
    path.join(os.tmpdir(), 'qwen-live-harness-host-'),
  );
  const archivePath = path.join(
    temporaryDirectory,
    'Qwen-Live-Harness-Host.zip',
  );
  const extractedPath = path.join(temporaryDirectory, 'extracted');
  const candidatePath = path.join(extractedPath, LIVE_HOST_APP_NAME);
  const stagingPath = `/Applications/.Qwen Live Harness Host.install-${randomUUID()}.app`;
  const backupPath = `/Applications/.Qwen Live Harness Host.backup-${randomUUID()}.app`;
