...
/** Help and startup errors remain readable when config is missing or invalid. */
export function readPreferredLiveLanguage(
  env: Record<string, string | undefined> = process.env,
): LiveLanguage {
  try {
    const value: unknown = JSON.parse(
      readFileSync(
        join(resolveLiveDataDirectory(env), 'config.json'),
        'utf8',
      ).replace(/^\uFEFF/u, ''),
    );
    if (
    ...
    /* The configuration loader reports errors after a locale is chosen. */
  }
  return 'en';
}
